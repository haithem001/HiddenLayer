...
class _SingleWalker:
    ...
    def _build(self, spawn_x=None, spawn_z=None, face_angle=0.0):
        ...
        if 'head' in joint_names:
            self.physics.kinematic_indices = {joint_names['head']}
        else:
            self.physics.kinematic_indices = set()

        # Pin head BEFORE WalkerEnv captures initial_points
        self._pin_head()
    ...
    def _pin_head(self):
        """Lock head directly above torso. Called before env init and after every step."""
        if 'head' not in self.joint_names:
            return
        ti = self.joint_names['torso']
        hi = self.joint_names['head']
        self.points[hi]['x'] = self.points[ti]['x']
        self.points[hi]['y'] = self.points[ti]['y'] + _NECK_LEN
    ...
    def get_obs_extended(self):
        ...
        # Gaze state encoded as two values:
        #   detected: 1.0 if target is in FOV, 0.0 while searching
        #   pursuing: 1.0 if actively pursuing, 0.0 otherwise
        detected = 1.0 if (self.gaze.state != 'searching') else 0.0
        pursuing = 1.0 if (self.gaze.state == 'pursuing')  else 0.0
    ...
    def step(self, action):
        ...
        self.isv.update(dt=0.008, env=hormone_env, events=hormone_events)
